...
data<- list(
  ...
  # coordinates = sf::st_coordinates(filtered_locations$pings),
...
pwg = make_starve_pred_graph(
  pred_coordinates = sim$pred_locs,
  field_coordinates = graph$coordinates
)
...
predictions<- sf::st_as_sf(
  data.frame(
    ...
    sf::st_geometry(pwg$coordinates)
  )
)
colnames(predictions)[1:4]<- c("dx", "dy", "dx_se", "dy_se")
...
colnames(mesh_predictions)[1:4]<- c("dx", "dy", "dx_se", "dy_se")
```

## Do some plotting

```{r}
# mesh_predictions[, 1:4]<- 1e9 * mesh_predictions[, 1:4, drop = TRUE]
...
tm_shape(sim$pred_field["w"][, , , 2:3]) + raster_style +
  tm_shape(mesh_predictions) + tm_dots(col = c("dx", "dy"), size = 0.2, style = "cont", midpoint = 0, palette = "PRGn") +
  tm_facets(nrow = 1, ncol = 2, free.scales = TRUE) + tm_layout(legend.outside = FALSE)
```
